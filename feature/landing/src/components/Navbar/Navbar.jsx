  ...
  return (
    <header className={`navbar ${scrolled ? 'navbar--scrolled' : ''}`} role="banner">
      <nav className="navbar__inner container" aria-label="Primary navigation">
        {/* Logo */}
        <Link to="/" className="navbar__logo" aria-label="GameTime — home">
          <FaGamepad className="navbar__logo-icon" aria-hidden="true" />
          <span className="navbar__logo-text">GameTime</span>
        </Link>
          ...
          {navLinks.map((link) => (
            <li key={link.to}>
              <NavLink
                to={link.to}
                className={({ isActive }) =>
                  `navbar__link ${isActive && link.to !== '/#features' ? 'navbar__link--active' : ''}`
                }
              >
                {link.label}
              </NavLink>
          ...
          {navLinks.map((link) => (
            <li key={link.to}>
              <NavLink
                to={link.to}
                className={({ isActive }) =>
                  `navbar__mobile-link ${isActive && link.to !== '/#features' ? 'navbar__mobile-link--active' : ''}`
                }
